    ...
    if step == "1":
        ...
        if isinstance(info, dict):
            ...
            print(profile % (info['account'], info['username'],
                             info['PASSWORD'], info['COUNT'],
                             info['province'], info['street'],
                             info['door'], info['bankname'],
                             info['money'], info['registerDate']))
        elif info == 2:
            print("该用户已存在")
            continue
        elif info == 3:
        ...
        if isinstance(flag, dict):
            ...
            while True:
                ...
                update("update user set money=%s where "
                ...
                if y:
                    ...
                    if acc2 != acc1:
                        ...
                        if yue < zhuan:
                            ...
                            update(
                                "update user set money=%s where "
                                "account=%s", (yue, acc1))
                            # 转入账户余额增加
                            y['money'] += zhuan
                            update("update user set money=%s where "
                                   "account=%s", (y['money'], acc2))
                            break
                    else:
        ...
        if isinstance(flag, dict):
            ...
            print(profile % (bank['account'], bank['username'],
                             bank['PASSWORD'], bank['COUNT'],
                             bank['province'], bank['street'],
                             bank['door'], bank['bankname'],
                             bank['money'], bank['registerDate']))
        elif flag == 2:
            print("密码错误！")
            continue
        elif flag == 3:
